...
static I2cErr hal_to_i2c_err(HAL_StatusTypeDef status)
{
    switch (status)
    {
        ...
        case HAL_TIMEOUT: return I2C_ERR_TIMEOUT;
        case HAL_BUSY:    return I2C_ERR_BUSY;
        default:          return I2C_ERR_HAL;
    }
}
                    ...
                    const uint8_t *data, uint16_t len, uint32_t timeout_ms)
{
    return hal_to_i2c_err(
        HAL_I2C_Master_Transmit(&i2c->hal_handle, dev_addr,
...
{
    return hal_to_i2c_err(
        HAL_I2C_Master_Receive(&i2c->hal_handle, dev_addr,
                               data, len, timeout_ms));
}
...
{
    return hal_to_i2c_err(
        HAL_I2C_Mem_Write(&i2c->hal_handle, dev_addr, mem_addr,
...
{
    return hal_to_i2c_err(
        HAL_I2C_Mem_Read(&i2c->hal_handle, dev_addr, mem_addr,
