...
{
    return hal_to_uart_err(HAL_UART_Transmit(&uart->hal_handle, (uint8_t *)data, len, timeout_ms));
}

UartErr uart_read(Uart *uart, uint8_t *data, uint16_t len, uint32_t timeout_ms)
{
    return hal_to_uart_err(HAL_UART_Receive(&uart->hal_handle, data, len, timeout_ms));
}
